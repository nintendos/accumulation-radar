...
def compute_three_strategies(conn):
    """
    OI + 费率 + 收筹 三策略与热度扫描。
    返回结构化 dict，供 Web API 与 format_three_strategies_telegram 使用。
    """
    watchlist = load_watchlist_symbols(conn)
    if not watchlist:
        return {"ok": False, "error": "标的池为空，先运行 pool 模式"}

    tickers_raw = api_get("/fapi/v1/ticker/24hr")
    premiums_raw = api_get("/fapi/v1/premiumIndex")
    if not tickers_raw or not premiums_raw:
        return {"ok": False, "error": "币安行情/费率 API 失败"}
    ...
    generated_at = now_cst.strftime("%Y-%m-%d %H:%M CST")
    ...
    for s in ambush_dark[:2]:
        highlights.append(f"🎯 {s['coin']} 暗流！OI{s['d6h']:+.0f}%但价格没动，市值仅{mcap_short_str(s['est_mcap'])}")
    ...
    for s in ambush_gem[:2]:
        if s["coin"] not in [h.split(" ")[1] for h in highlights]:
            highlights.append(f"💎 {s['coin']} 低市值{mcap_short_str(s['est_mcap'])}+OI{s['d6h']:+.0f}%，埋伏首选")

    hot_coins_sorted = sorted(
    ...
    return {
        ...
        "dual_heat": sorted(dual_heat),
        "vol_surge_count": len(vol_surge_coins),
        "mcap_count": len(mcap_map),
        "cg_trending_count": len(cg_trending),
        "hot_coins": hot_coins_sorted,
        "chase": chase,
        "combined": combined,
        "ambush": ambush,
...
def format_three_strategies_telegram(res):
    """由 compute_three_strategies 结果生成 Telegram 文本。"""
    if not res.get("ok"):
        return ""
    lines = [
        f"🏦 **庄家雷达** 三策略+热度",
    ...
    hot_coins = res.get("hot_coins") or []
    if hot_coins:
        lines.append(f"\n🔥 **热度榜** (CG趋势+成交量暴增)")
        for s in hot_coins[:8]:
            ...
            if oi_tag:
                tags.append(f"⚡{oi_tag}")
            if s["in_pool"]:
                tags.append(f"💤池{s['sw_days']}天")
            fr_tag = f"🧊{s['fr_pct']:.2f}%" if s["fr_pct"] < -0.03 else ""
            if fr_tag:
                tags.append(fr_tag)
            lines.append(
                f"  {s['coin']:<8} ~{mcap_short_str(s['est_mcap'])} 涨{s['px_chg']:+.0f}% | {' '.join(tags)}"
            )

    chase = res.get("chase") or []
    lines.append(f"\n🔥 **追多** (按费率排名)")
    if chase:
        for s in chase[:8]:
            lines.append(
                f"  {s['coin']:<7} 费率{s['fr_pct']:+.3f}% {s['trend']}"
                f" | 涨{s['px_chg']:+.0f}% | ~{mcap_short_str(s['est_mcap'])}"
            )
    else:
    ...
    for s in combined[:8]:
        dims = []
        if s["f_sc"] >= 10:
            dims.append(f"🧊{s['fr_pct']:.2f}%")
        if s["m_sc"] >= 12:
            dims.append(f"💎{mcap_short_str(s['est_mcap'])}")
        if s["s_sc"] >= 10:
            dims.append(f"💤{s['sw_days']}天")
        if s["o_sc"] >= 10:
            dims.append(f"⚡OI{s['d6h']:+.0f}%")
        lines.append(f"  {s['coin']:<7} {s['total']}分 | {' '.join(dims)}")

    ambush = res.get("ambush") or []
    lines.append(f"\n🎯 **埋伏** (市值35+OI30+横盘20+费率15)")
    for s in ambush[:8]:
        tags = [f"~{mcap_short_str(s['est_mcap'])}"]
        if abs(s["d6h"]) >= 2:
            tags.append(f"OI{s['d6h']:+.0f}%")
        if s["d6h"] > 2 and abs(s["px_chg"]) < 5:
            tags.append("🎯暗流")
        if s["sw_days"] >= 45:
            tags.append(f"横盘{s['sw_days']}天")
        if s["fr_pct"] < -0.01:
            tags.append(f"费率{s['fr_pct']:.2f}%")
...
def main():
    ...
    if mode in ("full", "oi"):
        res = compute_three_strategies(conn)
        if not res.get("ok"):
            err = res.get("error", "失败")
            if "标的池为空" in err:
                print(f"⚠️ {err}")
            else:
                print(f"❌ {err}")
            conn.close()
            return
        report = format_three_strategies_telegram(res)
        if report:
            send_telegram(report)
